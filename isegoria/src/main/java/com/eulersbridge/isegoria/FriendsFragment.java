...
public class FriendsFragment extends Fragment implements TitledFragment, MainActivity.TabbedFragment {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        isegoriaApp = (IsegoriaApp) mainActivity.getApplication();
    ...
    private void getFriends() {
        isegoriaApp.getAPI().getFriends().enqueue(new SimpleCallback<List<Contact>>() {
            @Override
    ...
    private void getSentFriendRequests() {
        ...
        isegoriaApp.getAPI().getFriendRequestsSent(userId).enqueue(new SimpleCallback<List<FriendRequest>>() {
            @Override
    ...
    private void getReceivedFriendRequests() {
        ...
        isegoriaApp.getAPI().getFriendRequestsReceived(userId).enqueue(new SimpleCallback<List<FriendRequest>>() {
            @Override
            protected void handleResponse(Response<List<FriendRequest>> response) {
    ...
    private boolean search(String query) {
        clearSearchResults();
        isegoriaApp.getAPI().searchForUsers(query).enqueue(searchCallback);
        ...
        if (getActivity() != null) {
            getActivity().runOnUiThread(() -> {
                ...
                if (type == UserType.SEARCH) {
                    ...
                    candidateProfileImage.setOnClickListener(view -> isegoriaApp.getAPI().addFriend(isegoriaApp.getLoggedInUser().email, user.email)
                ...
                if (type == UserType.FRIEND) {
                    ...
                    viewProfileImage.setOnClickListener(view -> {
                        ...
                        args.putParcelable(Constants.FRAGMENT_EXTRA_CONTACT, Parcels.wrap(user));
                ...
                if (friendRequestType == FriendRequestType.RECEIVED) {
                    ...
                    acceptImage.setOnClickListener(view -> {
                        ...
                        isegoriaApp.getAPI().acceptFriendRequest(contactRequestId).enqueue(new SimpleCallback<Void>() {
                            @Override
                            protected void handleResponse(Response<Void> response) {
                                showAcceptMessage();
                            }
                    ...
                    denyImage.setOnClickListener(view -> {
                        ...
                        isegoriaApp.getAPI().rejectFriendRequest(contactRequestId).enqueue(new SimpleCallback<Void>() {
                            @Override
                            protected void handleResponse(Response<Void> response) {
                                showDenyMessage();
                            }
